<script setup lang="ts">
import { ref, onMounted } from 'vue';
import { gsap } from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';

gsap.registerPlugin(ScrollTrigger);

/** 
 * Testimonials array.
 * You can add or remove as many as you want. 
 * Each testimonial has a name and text.
 */
const testimonials = ref([
  {
    name: 'John Smith',
    text: '“NorthStar Assistance has been a lifesaver for our family. Their care is unmatched.”',
  },
  {
    name: 'Sarah Johnson',
    text: '“They made a real difference in my mother’s daily routine. Highly recommend!”',
  },
  {
    name: 'Mike Thompson',
    text: '“Professional and compassionate from day one. We couldn’t ask for more.”',
  },
]);

/** 
 * Current testimonial index to display.
 */
const currentTestimonialIndex = ref(0);

/** 
 * Rotate testimonials every 10 seconds.
 */
onMounted(() => {
  // Basic timeline for the sections.
  const tl = gsap.timeline({ defaults: { duration: 1, opacity: 0 } });
  tl.from('.centerpiece', { y: 50 })
    .from('.who-we-are', { y: 50, opacity: 0 }, '+=0.3')
    .from('.services-section', { y: 50, opacity: 0 }, '+=0.3')
    .from('.testimonials-section', { y: 50, opacity: 0 }, '+=0.3')
    .from('.contact-section', { y: 50, opacity: 0 }, '+=0.3');

  // Simple interval to rotate testimonials
  setInterval(() => {
    currentTestimonialIndex.value =
      (currentTestimonialIndex.value + 1) % testimonials.value.length;
  }, 10000); // every 10 seconds
});
</script>

<template>
  <!-- Parallax Hero Section -->
  <section id="home" class="parallax">
    <div class="centerpiece">
      <img src="/src/assets/NSA.jpg" alt="Minnesota Landscape" class="logo" />
      <h1>Welcome to NorthStar Assistance</h1>
      <p>Providing quality PCA services tailored to your needs.</p>
    </div>
  </section>

  <!-- Who We Are (Alternating layout: text left, image right) -->
  <section class="content-block who-we-are">
    <div class="block-text">
      <h2>Who We Are</h2>
      <p>
        NorthStar Assistance specializes in providing compassionate and
        personalized home care for individuals in need. Our team is dedicated to
        ensuring comfort, safety, and a high quality of life for our clients.
      </p>
    </div>
    <div class="block-image">
      <!-- Replace placeholder with an actual image relevant to your “Who We Are” -->
      <img
        src="https://via.placeholder.com/600x400.png?text=Who+We+Are"
        alt="Who We Are"
      />
    </div>
  </section>

  <!-- Our Services (reverse layout: image left, text right) -->
  <section class="content-block services-section reverse">
    <div class="block-image">
      <!-- Replace placeholder with your services image -->
      <img
        src="https://via.placeholder.com/600x400.png?text=Our+Services"
        alt="Our Services"
      />
    </div>
    <div class="block-text">
      <h2>Our Services</h2>
      <p>
        We provide a wide range of home care services, including personal care,
        companionship, and specialized medical support tailored to each client.
        Whether you need a few hours a day or 24/7 assistance, our flexible
        programs can be customized to fit your lifestyle.
      </p>
    </div>
  </section>

  <!-- Testimonials Slider -->
  <section class="content-block testimonials-section">
    <h2>Testimonials</h2>
    <!-- 
      We'll just show one testimonial at a time, determined by currentTestimonialIndex.
      A simple fade transition can be added for smoothness.
     -->
    <transition name="fade">
      <div
        key="testimonial-{{ currentTestimonialIndex }}"
        class="testimonial-slide"
      >
        <blockquote>
          {{ testimonials[currentTestimonialIndex].text }}
        </blockquote>
        <cite>- {{ testimonials[currentTestimonialIndex].name }}</cite>
      </div>
    </transition>
  </section>

  <!-- Contact / CTA -->
  <section class="content-block contact-section">
    <h2>Get in Touch</h2>
    <p>
      Ready to get started? Contact us today to learn more about how we can
      support your needs.
    </p>
    <!-- You could place a button linking to your Contact page, or a phone #, etc. -->
    <router-link to="/contact" class="contact-btn">Contact Us</router-link>
  </section>
</template>

<style scoped>
/* Parallax (Hero) Section */
.parallax {
  height: 100vh;
  background: url('/src/assets/lights.jpg') no-repeat center center fixed;
  background-size: cover;
  display: flex;
  align-items: center;
  justify-content: center; ;
  margin: 0;
  padding: 0;
}

.centerpiece {
  background: rgba(0, 0, 0, 0.5);
  padding: 2em;
  border-radius: 10px;
  text-align: center;
  max-width: 800px;
  width: 90%;
  color: #fff;
}

.logo {
  max-width: 500px;
  margin-bottom: 1em;
}

/* Content Blocks (who we are, services, testimonials, contact) */
.content-block {
  display: flex;
  align-items: center;
  justify-content: space-between;
  max-width: 1200px;
  margin: 3em auto;
  padding: 2em;
  background-color: #545555;
  border-radius: 10px;
  box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);
  flex-wrap: wrap; /* so text and image stack on smaller screens */
  gap: 2rem;
}

.content-block.reverse {
  flex-direction: row-reverse; /* image on left, text on right */
}

.block-text {
  flex: 1;
  min-width: 300px;
}

.block-text h2 {
  font-size: 2.5rem;
  margin-bottom: 1em;
  color: #f29f05;
}

.block-text p {
  font-size: 1.2rem;
  color: #eee;
  line-height: 1.5;
}

.block-image {
  flex: 1;
  min-width: 300px;
  text-align: center;
}

.block-image img {
  width: 100%;
  max-width: 600px;
  border-radius: 8px;
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
}

/* Testimonials */
.testimonials-section {
  text-align: center;
}

.testimonial-slide {
  margin-top: 1em;
}

blockquote {
  font-style: italic;
  font-size: 1.3rem;
  line-height: 1.5;
  color: #f1f1f1;
}

cite {
  display: block;
  margin-top: 1em;
  color: #ddd;
  font-size: 1.1rem;
}

/* Fade transition for testimonial slides */
.fade-enter-active, .fade-leave-active {
  transition: opacity 0.6s;
}
.fade-enter, .fade-leave-to {
  opacity: 0;
}

/* Contact Section */
.contact-btn {
  display: inline-block;
  margin-top: 1em;
  padding: 0.75em 1.5em;
  background-color: #f29f05;
  color: #000;
  text-decoration: none;
  font-weight: bold;
  border-radius: 5px;
  transition: background 0.3s;
}

.contact-btn:hover {
  background-color: #ffb53f;
}

/* Responsive Adjustments */
@media (max-width: 768px) {
  .parallax {
    background-attachment: scroll; /* disable fixed parallax on mobile for performance */
  }

  .content-block {
    margin: 1em auto;
    padding: 1em;
  }

  .block-text h2 {
    font-size: 2rem;
  }
  .block-text p {
    font-size: 1rem;
  }
  blockquote {
    font-size: 1rem;
  }
}
</style>
